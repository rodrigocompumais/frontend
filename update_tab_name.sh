...
if [[ -z "$REACT_APP_TAB_NAME" ]]; then
    echo -e "${YELLOW}⚠️  REACT_APP_TAB_NAME não definida, usando valor padrão: Compuchat${NC}"
    TAB_NAME="Compuchat"
else
...
# Substitui todas as ocorrências de "Compuchat" pelo novo nome
sed -i "s/Compuchat/$TAB_NAME/g" public/index.html
...
if [[ -f "public/manifest.json" ]]; then
    sed -i "s/Compuchat/$TAB_NAME/g" public/manifest.json
